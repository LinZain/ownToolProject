...
public class SqlApp {
    private static Logger log = Logger.getLogger(SqlApp.class);
    ...
    public static void main(String[] args) throws IOException {
        ...
        //查询表的结构信息
        String sql2 = "SELECT ordinal_position,column_name,column_type, column_key, extra ,is_nullable, column_default, column_comment,data_type,character_maximum_length,NUMERIC_PRECISION "
                + "FROM information_schema.columns WHERE table_schema='" + map.get("-n") + "' and table_name='";

//
//        String sql1 = "select ut.table_name as table_name,ut.tablespace_name as engine,ut.buffer_pool as table_collation, uc.table_type as table_type,uc.comments as table_comment,ut.last_analyzed as create_options from user_tables ut,user_tab_comments uc where ut.table_name=uc.table_name";
//
//        String sql2 = "select rownum as ordinal_position,c.nullable as is_nullable,c.data_default as column_default,c.data_type as data_type,c.data_length as character_maximum_length,t.column_name as column_name,t.comments as column_comment from user_col_comments t,user_tab_columns c where c.column_name=t.column_name and c.table_name=t.table_name and t.table_name='";
//
    ...
    private static RowRenderData getHeader() {
        RowRenderData header = RowRenderData.build(
                ...
                new TextRenderData("字段名", POITLStyle.getHeaderStyle()),
        ...
        try {
            ...
            while (set.next()) {
                ...
                RowRenderData row = RowRenderData.build(
                        ...
                        new TextRenderData(set.getString("data_type").equals("int") ? set.getString("NUMERIC_PRECISION") + "" : set.getString("character_maximum_length") + ""),
                        new TextRenderData(set.getString("data_type").equals("bigint") ? set.getString("NUMERIC_PRECISION") + "" : set.getString("character_maximum_length") + ""),
                        new TextRenderData(set.getString("data_type").equals("datetime") ? "" : set.getString("character_maximum_length") + ""),
                        new TextRenderData(set.getString("is_nullable") + ""),
                        new TextRenderData(set.getString("column_comment") + "")
                        //new TextRenderData(set.getString("column_default")!=null?set.getString("column_default"):"")
                );
                if (i % 2 == 0) {
